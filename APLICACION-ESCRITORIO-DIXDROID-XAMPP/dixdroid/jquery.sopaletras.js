(function ($) {
    Sopa = function ($el, options) {
        ...
        var defaults = { palabras: [{ name: 'BANCO' },{ name: 'CAÑA' },{ name: 'ARAÑA' },{ name: 'CABAÑA' }, { name: 'PERRO' }, { name: 'CABALLO' }, { name: 'GATO' },{ name: 'BARCO'}], complejo: 11, vertical: 'S', onWin: "" };
        var aciertos = 0;
        ...
        this.init = function () {
            ...
            for (var j = 0; j < defaults.complejo + 1; j++) {
                ...
                for (var i = 0; i < defaults.complejo + 1; i++) {
                    ...
                    var letraelegidapos = Math.floor(Math.random() * 12);
            ...
            if (cantidadclicks == 1) {
                ...
                $.each(defaults.palabras, function () {
                    if (selecion == this.name) {
                        ...
                        alertify.alert("Encontraste la palabra: " + selecion);
                        if (!verificar) {
                        ...
                        if (aciertos == defaults.palabras.length) {
                           alertify.alert("Felicitaciones!!! Has encontrado todas las palabras.");
                            $g.onWin();
                        }
                    }
                });
